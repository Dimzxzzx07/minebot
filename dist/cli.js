...
function helper(messages, delayMs) {
...
async function runSpamBots(config) {
    ...
    if (config.autoChat)
        console.log(`${colors.cyan}Auto Chat:${colors.reset} ${config.autoChat}`);
    console.log(``);
    if (config.debug)
        (0, index_1.setLogLevel)(index_1.LogLevel.DEBUG);
    const bots = [];
    const failedBots = [];
    ...
    for (let i = 0; i < totalBots; i++) {
        ...
        try {
            const bot = await (0, index_1.createBot)(botOptions);
            bots.push(bot);
            bot.on('ready', () => {
...
async function runOSINTMode(config) {
    ...
    console.log(`${colors.cyan}Username:${colors.reset} ${config.username}`);
    if (config.watchlist)
        console.log(`${colors.cyan}Watchlist:${colors.reset} ${config.watchlist}`);
    if (config.logFile)
        console.log(`${colors.cyan}Log File:${colors.reset} ${config.logFile}`);
    console.log(``);
    (0, index_1.setLogLevel)(index_1.LogLevel.OSINT);
    const botOptions = {
        target: config.target,
        port: parseInt(config.port),
        version: '1.20.4',
    ...
    try {
        ...
        if (config.watchlist && playerTracker) {
            ...
            players.forEach((p) => playerTracker.addToWatchlist(p.trim()));
...
async function runStressTest(config) {
    const { StressTester } = await Promise.resolve().then(() => __importStar(require('./behaviors/StressTester')));
...
async function runConfigFile(filePath) {
